    ...
    <div class="content-box shadow overflow-hidden">
        <div class="d-flex">
            ...
            <div class="ms-4 w-100">
                ...
                <?php else : ?>
                    <h3 class="m-0"><?= normalize_price($product->price) . 'R$' ?>/<span class="text-primary"><strong><?= normalize_price(calculate_promotion($product->price, $product->promotion)) . 'R$' ?></strong></span></h3>
                <?php endif; ?>
                ...
                <div class="text-end align-items-bottom">
                    <a href="<?= site_url('products/edit/' . Encrypt($product->product_id)) ?>" class="btn btn-sm btn-outline-secondary px-3 m-1"><i class="fa-regular fa-pen-to-square me-2"></i>Editar</a>
                    <a href="<?= site_url('stocks/movements/' . Encrypt($product->product_id)) ?>" class="btn btn-sm btn-outline-secondary px-3 m-1"><i class="fa-solid fa-cubes-stacked me-2"></i>Stock</a>
                    <a href="<?= site_url('products/delete/' . Encrypt($product->product_id)) ?>" class="btn btn-sm btn-outline-secondary px-3 m-1"><i class="fa-regular fa-trash-can me-2"></i>Eliminar</a>
                </div>
            </div>
        </div>
    </div>
